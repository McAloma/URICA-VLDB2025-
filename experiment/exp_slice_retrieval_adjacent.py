...
class Inter_Retrieval_experiment():
    ...
    def main(self, args, region_retriever):
        ...
        for query_slice_info in tqdm(query_slice_infos):
            ...
            start = time.time()
            _, _, top_targets = region_retriever.retrieve(query_slice)
            end = time.time()

            names = [item[0] for item in top_targets]
        ...
        self.save_results_to_file({f"preprocess={args.preprocess}, evaluation={args.evaluation}, date={current_date}": final_resuls},
                            filename="experiment/results/slice_retrieval_adjacent_results.txt")
        
    def save_results_to_file(self, results, filename="./results/slice_retrieval_adjacent_results.txt"):
